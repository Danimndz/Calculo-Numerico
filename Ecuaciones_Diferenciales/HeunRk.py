...
class Heun:
    def __init__(self):
        self.heun_ = Tk()
        self.heun_.geometry('850x450')
        self.heun_.title("Heun Method")
        self.heun_.resizable(width=False, height=False)
        fuente = font.Font(weight='normal')

        self.imag2 = ImageTk.PhotoImage(Image.open(path2))
        self.img2 = ttk.Label(self.heun_,image = self.imag2,width=400)

        self.fx=StringVar()
        self.fx_label = ttk.Label(self.heun_,text='f(x) =',font=fuente)
        self.fxBox = ttk.Entry(self.heun_,textvariable=self.fx,width=30)

        self.a_ = StringVar()
        self.a_box = ttk.Entry(self.heun_,textvariable=self.a_,width=5)

        self.b_ = StringVar()
        self.b_box = ttk.Entry(self.heun_,textvariable=self.b_,width=5)

        self.y_ = StringVar()
        self.y_label = ttk.Label(self.heun_,text='yinicial =',font=fuente)
        self.y_box = ttk.Entry(self.heun_,textvariable=self.y_,width=5)

        self.h_ = StringVar()
        self.h_label = ttk.Label(self.heun_,text='h =',font=fuente)
        self.h_box = ttk.Entry(self.heun_,textvariable=self.h_,width=5)

        self.display_txt = Text(self.heun_,width=100,height=10)
        self.taylor_btn = ttk.Button(self.heun_,text='wi',command=self.Heun_)
        self.help_btn = ttk.Button(self.heun_,text="Help",command=self.help_ )
        self.back_mainM = ttk.Button(self.heun_,text="Return to D.E Menu",command=self.back_main)
        ...
        self.h_label.place(x=260,y=130)
    ...
    def help_(self):
        self.newWindow = Toplevel(self.heun_)
    ...
    def Heun_(self):
        try:
            ...
            def euler(a_,b_,h_,y0_):
                ...
                for t_ in ti:
                    tab.add_row([t_,w,f(t_,w),f(t_+(2/3*h_),w+2/3*h_*f(t_,w))])
                    w = w+(h/4*(f(t_,w)+3*f(t_+(2/3*h_),w+2/3*h_*f(t_,w))))

            tab = PrettyTable()
            tab.field_names=(['ti','wi','f(t,w)','f(t,w)+3*f(t+((2/3)*h),w+(2/3)*h*f(t,w)'])
            fx = self.fxBox.get()
            t = symbols('t')
            y = symbols('y')
